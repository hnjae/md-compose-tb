...
`browser.compose.onBeforeSend`는 send flow를 가로채고 갱신된 compose details를 반환할 수 있다. 따라서 전송 시점 Markdown 파이프라인에 적합하다. Markdown source를 읽고, sanitized HTML을 렌더하고, 원본 Markdown을 `plainTextBody`로 유지한 뒤 Thunderbird가 메시지를 보내기 전에 갱신된 `details`를 반환할 수 있다.
...
- Plain fallback: `details.plainTextBody`를 원본 Markdown source로 설정한다.
